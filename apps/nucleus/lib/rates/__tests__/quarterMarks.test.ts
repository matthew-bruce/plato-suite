...
import { generateQuarterMarks, shouldShowQuarterLabel } from '../quarterMarks'
...
describe('generateQuarterMarks', () => {
  it('labels Apr–Dec dates as "this calendar year → next" (1 Jul 2023 = Q2 FY 23/24)', () => {
    ...
    expect(marks).toEqual([{ ms: Date.UTC(2023, 6, 1), label: 'Q2 FY 23/24', quarter: 2 }])
  })
  ...
  it('labels Jan–Mar dates as "previous calendar year → this" (1 Jan 2026 = Q4 FY 25/26)', () => {
    ...
    const marks = generateQuarterMarks(start, end)
    expect(marks).toEqual([{ ms: Date.UTC(2026, 0, 1), label: 'Q4 FY 25/26', quarter: 4 }])
  })
  ...
  it('excludes boundaries outside the window on both edges', () => {
    ...
    expect(marks).toEqual([{ ms: Date.UTC(2024, 6, 1), label: 'Q2 FY 24/25', quarter: 2 }])
  })
  ...
  it('a boundary landing exactly on the window edge is included (inclusive range)', () => {
    ...
    expect(marks).toEqual([{ ms: boundary, label: 'Q4 FY 24/25', quarter: 4 }])
  })
...
describe('shouldShowQuarterLabel — label thinning on long windows', () => {
  it('1Y (4 marks): every quarter is labelled', () => {
    for (const q of [1, 2, 3, 4] as const) expect(shouldShowQuarterLabel({ quarter: q }, 4)).toBe(true)
  })

  it('3Y (12 marks): every quarter is labelled', () => {
    for (const q of [1, 2, 3, 4] as const) expect(shouldShowQuarterLabel({ quarter: q }, 12)).toBe(true)
  })

  it('5Y (20 marks): every quarter is still labelled — the threshold is inclusive', () => {
    for (const q of [1, 2, 3, 4] as const) expect(shouldShowQuarterLabel({ quarter: q }, 20)).toBe(true)
  })

  it('All time (32+ marks, 8+ years): only Q1 (1 Apr, financial-year start) is labelled', () => {
    expect(shouldShowQuarterLabel({ quarter: 1 }, 32)).toBe(true)
    expect(shouldShowQuarterLabel({ quarter: 2 }, 32)).toBe(false)
    expect(shouldShowQuarterLabel({ quarter: 3 }, 32)).toBe(false)
    expect(shouldShowQuarterLabel({ quarter: 4 }, 32)).toBe(false)
  })

  it('just past the threshold (21 marks) also thins to Q1 only', () => {
    expect(shouldShowQuarterLabel({ quarter: 1 }, 21)).toBe(true)
    expect(shouldShowQuarterLabel({ quarter: 2 }, 21)).toBe(false)
  })

  it('thinning a real "All time" window still keeps a dashed line for every quarter (count unchanged), while only Q1 marks are labelled', () => {
    ...
    const labelled = marks.filter((m) => shouldShowQuarterLabel(m, marks.length))
    // Every labelled mark is a Q1 (1 Apr) boundary...
    expect(labelled.every((m) => m.quarter === 1)).toBe(true)
    // ...and every Q1 boundary present in the window is labelled.
    expect(labelled.length).toBe(marks.filter((m) => m.quarter === 1).length)
    // The full set of dashed lines is untouched by thinning.
    expect(marks.length).toBeGreaterThan(labelled.length)
  })
})
