...
export function PlatformDetailView({ platform }: Props) {
  ...
  const [teamModal, setTeamModal] = useState<TeamModalMode>({ kind: 'closed' })
  ...
  return (
    ...
    >
      ...
      <section style={{ marginBottom: 'var(--rmg-spacing-11)' }}>
        ...
        >
          ...
          <Button
            ...
            size="medium"
            background="light"
            onClick={() => setTeamModal({ kind: 'create' })}
          >
            Add Team
          </Button>
        </div>
            ...
            {platform.teams.map((team) => {
              ...
              return (
                <TeamCard
                  ...
                  onEdit={() => setTeamModal({ kind: 'edit', team })}
                />
              )
            })}
          </div>
      ...
      {/* Team modal */}
      {teamModal.kind !== 'closed' && (
        <TeamModal
          mode={teamModal}
          platformId={platform.platform_id}
          platformSlug={platform.platform_slug}
          workstreams={platform.workstreams}
          onClose={() => setTeamModal({ kind: 'closed' })}
        />
      )}
    </div>
  )
